...
# Run LLM query and update buffer
_zsh_ai_run_query() {
    local query="$1"
    local saved_buffer="$2"
...
_zsh_ai_accept_line() {
    ...
    if [[ "$BUFFER" =~ ^'# ' ]]; then
        ...
        _zsh_ai_run_query "$query" "$saved_buffer"
    else
...
# Custom widget to send current buffer
_zsh_ai_send_buffer() {
    local query="$BUFFER"
    local saved_buffer="$BUFFER"
...
_zsh_ai_init_widget() {
    _zsh_ai_do_init() {
        ...
        zle -N zsh-ai-send-buffer _zsh_ai_send_buffer
        bindkey '^O' zsh-ai-send-buffer
